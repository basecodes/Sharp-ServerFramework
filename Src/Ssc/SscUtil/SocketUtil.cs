...
#if Win32
        public static IPEndPoint createIpEndPoint(string serverName, int portNumber) {
            var hostInfo = Dns.GetHostEntry(serverName);
            var serverAddress = hostInfo.AddressList[0];
            return new IPEndPoint(serverAddress, portNumber);
        }
